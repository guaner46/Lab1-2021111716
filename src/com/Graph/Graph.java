...
// 成员 ////////////////////////////////////////////////////
    ...
    private ArrayList<HashMap<Integer, Integer>> textMap;
    /**
     * The Idx 2 text.
     */
// 建立一个从索引到对应单词的映射。
    private HashMap<Integer, String> idx2Text;
    /**
     * The Text 2 idx.
     */
// 建立一个从单词到索引的映射。
    private HashMap<String, Integer> text2Idx;
    /**
     * The Input words.
     */
     ...
     * @return 是否成功创建索引
     */
    private boolean constructTextMapping(final String text) {
     ...
     * @return 单词对应的索引值
     */
    private int getIdx(final String word) {
     ...
     * @return 索引对应的单词
     */
    private String getTag(final int idx) {
     ...
     * @return 结构化的层结构字符串
     */
    private String printLevelInformation(final int level) {
     ...
     * @return 是否成功插入
     */
    private boolean insertFirstWord(final String currWord) {
     ...
     * @return 是否成功插入
     */
    private boolean insertPostWord(
            final String prevWord, final String currWord) {
        if (constructTextMapping(currWord)) {
            textMap.add(new HashMap<>());
        }
     ...
     * @return 是否有对应的有向边
     */
    private boolean isConnected(final String source, final String target) {
     ...
     * @return 是否有对应的有向边
     */
    private boolean isConnected(final int srcIdx, final int tgtIdx) {
     ...
     * @return 结构化的字符串
     */
    private String printBridgeList(final ArrayList<String> a) {
        String result = "";
        for (String b : a) {
            result += (" " + b);
        }
     ...
     */
    public boolean constructGraph(final ArrayList<String> words) {
        if (words.isEmpty()) {
    ...
    public String generateNewText(final String inputText) {
        ...
        for (int idx = 1; idx < words.length; ++idx) {
            ArrayList<String> bridgeWords =
                    queryBridgeWordsSolver(prevWord, words[idx]);
            if (bridgeWords.isEmpty()) {
                ...
                result += " "
                        +
                        bridgeWords.get(getRandomNumber()) + " " + words[idx];
            }
            prevWord = words[idx];
        }
        return result;
     ...
     * @param savename
     */
    public void showDirectedGraph(final String savename) {
        int[] prev = null;
     ...
     * @param savename
     */
    public void showDirectedGraph(final String savename, final int[] prev) {
        showDirectedGraph(savename, 800, 800, 200.0, 400, 400, prev);
    }
    /**
     * .
     ...
     */
    public void showDirectedGraph(
            final String savename, int frameW, int frameH,
            Double radius, double centerX,
            double centerY, final int[] prev) {
        ...
        if (scale < 1) {
            ...
            centerX = (double) frameW / 2;
            centerY = (double) frameH / 2;
            radius = frameH - centerY - 70;
        }
        JFrame frame = new JFrame(savename);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(frameW, frameH);

        List<Point2D> points = DrawGraph.layoutPointsOnCircle(
                radius, numPoints, centerX, centerY);
        ...
        for (int i = 0; i < textMap.size(); i++) {
            if (i != index1) {
                ...
                if (prev[curr] == -1) {
                    ...
                    // 显示有向图，使用新的前驱数组
                    showDirectedGraph(
                            word1 + "_" + getTag(i) + "_output.png", prev2);
                    reText = reText + result + "\n";
                }
            }
        }
        return reText;
    ...
    public String randomWalk() {
        ...
        System.out.println("input 'c' to continue;"
                +
                " input 'b' to break; input 'a' to auto.");
        ...
        int visitedFlag = 0;  // 跳出标记
        int autoFlag = 0;  // 自动游走标记
        ...
        while (true) {
            ...
            if (Objects.equals(input, "b")) {
                ...
                HashMap<Integer, Integer> currMap =
                        textMap.get(curr);  // 当前节点的邻接表
                ...
                for (int key : currMap.keySet()) {
                    if (i == next) {  // 随机获得的下一个节点
                        curr = key;
                        Tuple newtuple = new Tuple(oldCurr, curr);  // 新的边
                        for (Tuple t : visited) {
                ...
                if (visitedFlag == 0) {
                    visited.add(new Tuple(oldCurr, curr));
    ...
    static class Tuple {
        ...
        Tuple(final int input1, final int input2) {
            this.element1 = input1;
            this.element2 = input2;
        }

        public boolean isequal(final Tuple t) {
     ...
     * @return 前驱数组
     */
    public int[] dijkstra(final int index1) {
        // dijkstra 最短路
        int nodeSize = textMap.size();
        ...
        for (int i = 0; i < nodeSize; ++i) {
            ...
            for (int j = 0; j < nodeSize; ++j) {
                if (!visited[j] && isConnected(minIndex, j)
                        &&
                        dist[minIndex]
                                +
                                textMap.get(minIndex).get(j) < dist[j]) {
     ...
     */
    public void saveString(final String texToSave) {
        // 获取当前时间
        LocalDateTime currentTime = LocalDateTime.now();
     ...
     * 保存文本到文件
     *
     * @param textToSave 欲保存的文本
     * @param savename 保存的文件名
     */
    public void saveString(final String textToSave, final String savename) {
        try {
     ...
     */
    public void savePic(final JFrame jf, final String savename) {
        //得到窗口内容面板
...
// 构造器 ///////////////////////////////////////////////////
     ...
     */
    public Graph(final ArrayList<String> words) {
        textMap = new ArrayList<>();
        idx2Text = new HashMap<>();
        text2Idx = new HashMap<>();
        inputWords = words;
     ...
     * Sets text map.
     *
     * @param hashMaps the text map
     */
    public void setTextMap(
            final ArrayList<HashMap<Integer, Integer>> hashMaps) {
        this.textMap = hashMaps;
    }
     ...
     * Sets idx 2 text.
     *
     * @param idxToText the idx 2 text
     */
    public void setIdx2Text(final HashMap<Integer, String> idxToText) {
        this.idx2Text = idxToText;
    }
     ...
     * Sets text 2 idx.
     *
     * @param textToIdx the text 2 idx
     */
    public void setText2Idx(final HashMap<String, Integer> textToIdx) {
        this.text2Idx = textToIdx;
    }
}
